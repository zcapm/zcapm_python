...
class Testing():
    ...
    def __init__(self,print_progress = True):
        ...
        port_stream = pkg_resources.resource_stream(__name__,"data/ff25_day.csv")
        factors_stream = pkg_resources.resource_stream(__name__,"data/mu_sigma.csv")
        ff_factors_stream = pkg_resources.resource_stream(__name__,"data/ff_factors.csv")
        ind47_stream = pkg_resources.resource_stream(__name__,"data/ind47.csv")

        ports = pd.read_csv(port_stream,index_col = 'Date')
        factors = pd.read_csv(factors_stream,index_col = 'Date')
        ff_factors = pd.read_csv(ff_factors_stream,index_col = 'Date')
        ind47 = pd.read_csv(ind47_stream,index_col = 'Date')

        #convert indices to datetime objects
        ports.index = pd.to_datetime(ports.index)
        factors.index = pd.to_datetime(factors.index)
        ff_factors.index = pd.to_datetime(ff_factors.index)
        ind47.index = pd.to_datetime(ind47.index)
        ...
        ind47 = ind47.loc['1964-01-02':'2015-12-31',:]
        ...
        ind47 = ind47.sub(factors.R_f,axis = 'rows')
        ...
        ind47.insert(0,'YearMonth',ind47.index.strftime('%Y')+factors.index.strftime('%m'))
        ...
        ind47.YearMonth = ind47.YearMonth.astype(int)
        ...
        ind_monthports = (ind47.iloc[:,1:]/100)+1
        ind_monthports.insert(0,'YearMonth',ind47.YearMonth)
        ind_monthports = ind_monthports.groupby('YearMonth').prod()
        ind_monthports = (ind_monthports -1)*100

        self.sorted_portfolio_daily_excess_return = ports
        self.factor_daily_return = factors
        self.sorted_portfolio_monthly_excess_return = monthports
        self.industry_portfolio_daily_excess_return = ind47
        self.industry_portfolio_monthly_excess_return = ind_monthports
    
    def _EMRegression(self,dates,port_exc_ret, mkt_exc_ret, mkt_sigma, tol, MaxIter, criterion):
        '''
        Runs the _EM_loop
        ...
        -------
            1
            This method does not return anything important. Rather this method calls _EM_loop which adds estimation
        ...
        #Calls the _EM_loop method
        self._EM_loop(beta_hat,Z_hat,sigma_sq_hat,p_hat,port_exc_ret,mkt_exc_ret,mkt_sigma,tol,MaxIter,criterion,beta_zeta_cols,date)
        return 1
    
    def _EM_loop(self, beta_hat, Z_hat, sigma_sq_hat, p_hat, port_exc_ret, mkt_exc_ret, mkt_sigma, tol, MaxIter, criterion,beta_zeta_cols,date):
        '''
    ...
    def _LMRegression(self,dates,port_exc_ret,factors):
        '''
    ...
    def _rollapplyLM(self,port_exc_ret,factors,width):
        '''
        Creates a rolling window for the current portfolio and calls _LMRegression
        ...
        '''
        #create a dates series to roll on and apply the _LMRegression function
        dates = pd.Series(port_exc_ret.index.unique()).iloc[:-1]
        if self.print_progress:
            print('Fitting linear model for '+port_exc_ret.name,end='\r')
        dates.rolling(width).apply(self._LMRegression,args = (port_exc_ret,factors))
        return 1
    
    def estLinearModel(self,port_exc_ret,factors,width):
        '''
        Calls the _rollapplyLM function for applying the _LMRegression function
        ...
        #apply the _rollapplyLM function to each column of the portfolio returns
        port_exc_ret.apply(self._rollapplyLM,axis = 0,args = (factors,width))
        
        return self.results
    
    def _rollapplyEM(self,port_exc_ret,mkt_exc_ret,mkt_sigma,width,tol,MaxIter,criterion):
        '''
        Creates a rolling window for the current portfolio and calls _EMRegression
        ...
        '''
        #create a data pandas series to roll on and apply the _EMRegression method
        dates = pd.Series(port_exc_ret.index.unique()).iloc[:-1]
        if self.print_progress:
            print('Fitting ZCAPM model for '+port_exc_ret.name,end='\r')
        dates.rolling(width).apply(self._EMRegression,args = (port_exc_ret, mkt_exc_ret,mkt_sigma,tol,MaxIter,criterion))
    
    def estZCAPM(self,port_exc_ret,mkt_exc_ret,mkt_sigma,tol,MaxIter,criterion,width):
        '''
        Calls the _rollapplyEM function for applying the _EMRegression function
        ...
        #apply the _rollapplyEM function to each column of portfolio returns
        port_exc_ret.apply(self._rollapplyEM,axis = 0,args = (mkt_exc_ret,mkt_sigma,width,tol,MaxIter,criterion))
